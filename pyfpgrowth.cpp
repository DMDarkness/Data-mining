...
Header *FreItem(double minsup, PyObject* pyDataset)//Scanning the whole dataset once and output the headTable
{
	memset(Items, 0, BUFFER * sizeof(double));
	itemsize = 0;
	long value;
	long i,j;
	for (i = 0; i < transNum; i++)//Counting the support of every item
	{
		PyObject* trans;
...
Node* FPtree(Header *Head, PyObject* pyDataset)//Build FP-tree
{
...
long fpgrowth(double minsup, PyObject* pyDataset)
{
	Header *fphead;
	Node *fpTree;
	fphead = FreItem(minsup, pyDataset);//scanning the dataset and find those items with supports higher than minsup
	fpTree = FPtree(fphead, pyDataset);//building fp-tree
...
pyfpgrowth(PyObject *self, PyObject *args)
{
	...
	transNum = PyList_Size(pyDataset);
	long i, j;

	fpgrowth(minsup, pyDataset);//using fpgrowth to mine the results.
...
static PyMethodDef pyfpgrowthMethods[] = {
	{ "getFP", pyfpgrowth, METH_VARARGS,
"<fps>=getFP(<dataset>,<minsup>)\n\n\
input:\n\
